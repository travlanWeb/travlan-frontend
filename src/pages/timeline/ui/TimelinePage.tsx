    ...
    const handleSave = async (status: string) => {
        ...
        try {
            if (travelId && travelId !== 'new') {
                const payload = { name, totalBudget, status, startDate, endDate, bags, visits: formattedVisits, travelImage: finalTravelImage }
                const response = await api.put(`/travels/${travelId}`, payload)
                console.log("수정 성공", response.data)
            } else {
                const payload = { userId, name, totalBudget, status, startDate, endDate, bags, visits: formattedVisits, originalId, travelImage: finalTravelImage }
